...
- 애플리케이션 공통 템플릿은 `infra-shared/helm/`
- 로컬 dev 환경 차이는 `infra-local/environments/dev/`에서 override
...
helm upgrade --install jenkins-controller ../../../infra-shared/helm/jenkins-controller \
...
- `data/mysql-values.yaml`은 Bitnami MySQL 단일 인스턴스 기준 값입니다. 초기 스키마는 애플리케이션 Flyway migration이 생성합니다.
